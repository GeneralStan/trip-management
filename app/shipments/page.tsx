...
import {OrderFilterTab, ShipmentOrder, ShipmentTrip, TripFilterTab} from '@/types/shipment';
import {mockOrders, mockTrips} from '@/lib/mockShipments';
import {extractStringIdsFromOrders} from '@/lib/tripGeneration';
import {setStorageItem, getStorageItem, removeStorageItem, STORAGE_KEYS} from '@/lib/storage';
...
import {useServiceData} from "@/app/shipments/hooks/useServiceData";
import {useOrders} from "@/app/shipments/hooks/useOrders";
import {solveService} from "@/services/REST";
import {useTrips} from "@/app/shipments/hooks/useTrips";

type TabKey = "summary" | "orders" | "trips";
...
function ShipmentsContent() {
    const [toast, setToast] = useState<ToastState>(() => {
        ...
        return {show: false, status: 'success', message: ''};
    });
    const {
        dates,
        dispatchers,
        delivery_types,
        // is_loading
    } = useServiceData((status, msg) => {
        setToast({show: true, status, message: msg});
    })
    ...
    const [activeTab, setActiveTab] = useState<TabKey>(tabParam || 'summary');

    // Delivery Type filter state
    const [deliveryTypeFilter, setDeliveryTypeFilter] = useState<string | null>(
        // deliveryTypeParam || 'CORE'
        null
    );

    // Dispatcher filter state
    const [dispatcherFilters, setDispatcherFilters] = useState<string[]>(
        // dispatchersParam?.split(',').filter(d => d.trim()) ?? []
        []
    );
    ...
    const [tripsSortByOption, setTripsSortByOption] = useState('');
    ...
    const {
        ...
        deliveryDate ? dateToYYYYMMDD(deliveryDate) : undefined,
        dispatcherFilters.length === 0 ? undefined : dispatcherFilters.join(','),
    ...
    const filteredOrders = orders
        ...
        .sort((a, b) => {
            ...
            if (sortByOption === 'Orders') {
                valueA = a.order_count;
                valueB = b.order_count;
    ...
    const {trips, is_loading: trips_loading} = useTrips(
        deliveryDate ? dateToYYYYMMDD(deliveryDate) : '',
        (status, msg) => {
            setToast({show: true, status, message: msg});
        }
    )

    const generateTrips = React.useCallback(() => {
        if (!selectedOrders.size) return

        setIsGeneratingTrips(true)
        solveService({
            date: deliveryDate ? dateToYYYYMMDD(deliveryDate) : '',
            strings: extractStringIdsFromOrders(orders, selectedOrders),
        }).then(async res => {
            // Store trips in IndexedDB (handles large data > 5MB)
            await setStorageItem(STORAGE_KEYS.GENERATED_TRIPS, res.data ?? []);
            console.log("Generated Trips:", res.data)
            if (res.error) {
                setToast({show: true, status: "error", message: "Error generating trips: " + res.error});
            }
        }).finally(() => {
            setIsGeneratingTrips(false)
            router.push('/generate-trips');
        })
    }, [deliveryDate, orders, router, selectedOrders]);
    ...
    const filteredTrips = trips
        ...
        .sort((a, b) => {
            ...
            if (tripsSortByOption === 'Orders') {
                ...
                valueB = parseInt(a.tripId);
                valueA = parseInt(b.tripId);
            }
    ...
    const filteredDeliveryTypes = delivery_types.filter((type) => {
    ...
    const getSummaryForDeliveryType = (deliveryType: string) => {
        const orders = mockOrders.filter((order) => {
            const statusMatch = orderFilterTab === 'pending'
                ...
                : orderFilterTab === 'finalized'
                    ...
                    : true;
            return order.delivery_type === deliveryType && statusMatch;
        });

        const totalOrders = orders.reduce((sum, order) => sum + order.order_count, 0);
        ...
        const totalOutlets = orders.reduce((sum, order) => sum + order.capacity, 0);
        const totalCubes = orders.reduce((sum, order) => sum + order.capacity, 0);
        const totalCases = orders.reduce((sum, order) => sum + order.capacity, 0);
    ...
    const toggleSelectAllOrders = () => {
        if (selectedOrders.size === filteredOrders.length) {
            ...
            setSelectedOrders(new Set(filteredOrders.map((o) => o.string)));
        }
    };
    ...
    const handleGenerateTrips = async () => {
        ...
        const firstSelectedOrder = mockOrders.find(o => selectedOrders.has(o.string));
        const deliveryType = firstSelectedOrder?.delivery_type || 'CORE';
    ...
    return (
        <div className="flex h-screen w-screen bg-gray-50">
            ...
            <div className="flex flex-col flex-1">
                ...
                {/* Content */}
                <div className="flex-1 px-4 bg-white flex flex-col overflow-hidden"
                     style={{paddingTop: '20px', paddingBottom: activeTab !== 'summary' ? '16px' : '0'}}>
                    {/* Section Header */}
                                ...
                                {showMoreFiltersDropdown && (
                                    <div className="absolute top-full mt-2 z-50">
                                        <MoreFiltersDropdown
                                            ...
                                            deliveryTypes={delivery_types.map(dt => ({
                                                label: dt, value: dt
                                            }))}
                                            availableDispatchers={dispatchers}
                                ...
                                <div className="relative" ref={sortByDropdownRef}>
                                    ...
                                    >
                                        ...
                                        <ArrowDownwardOutlined sx={{
                                            fontSize: 16,
                                            color: '#374151',
                                            transform: sortByOrder === 'asc' ? 'rotate(180deg)' : 'none'
                                        }}/>
                                    </button>
                                    {showSortByDropdown && (
                                        <div className="absolute top-full mt-2 z-50">
                                ...
                                <div className="relative" ref={tripsSortByDropdownRef}>
                                    ...
                                    >
                                        ...
                                        <ArrowDownwardOutlined sx={{
                                            fontSize: 16,
                                            color: '#374151',
                                            transform: tripsSortByOrder === 'asc' ? 'rotate(180deg)' : 'none'
                                        }}/>
                                    </button>
                                ...
                                orderFilterTab === "pending" && (
                                    ...
                                    >
                                        {isGeneratingTrips ? (
                                            <>
                                                <div
                                                    className="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                                                Generating...
                                            </>
                                        ) : (
                                            <>
                                ...
                                <div className="pb-5">
                                    {filteredDeliveryTypes.length > 0 ? (
                                                    ...
                                                    <div className="bg-white py-4">
                                                        <div className="flex items-start">
                                                            ...
                                                            <div className="flex flex-col justify-between"
                                                                ...
                                                                <div>
                                                                    <h3 className="text-sm font-bold mb-2"
                                                                        style={{color: '#252525'}}>{deliveryType}</h3>
                                                                    <p className="text-xs mb-7"
                                                                       style={{color: '#6B7280'}}>Last
                                                                ...
                                                                >
                                                                    View All
                                                                    <NorthEastOutlined
                                                                        sx={{fontSize: 12, color: 'currentColor'}}/>
                                                                </button>
                                                            </div>
                                                            ...
                                                            <div className="flex-1 grid grid-cols-4 gap-3">
                                                                <div
                                                                    className="rounded-md p-4 flex flex-col justify-center"
                                                                    style={{
                                                                        backgroundColor: '#F5F5F5',
                                                                        height: '88px'
                                                                    }}>
                                                                    <p className="text-xs mb-1"
                                                                       style={{color: '#6B7280'}}>Total Orders</p>
                                                                    <p className="text-xl font-bold"
                                                                       style={{color: '#252525'}}>{summary.totalOrders}</p>
                                                                </div>
                                                                <div
                                                                    className="rounded-md p-4 flex flex-col justify-center"
                                                                    style={{
                                                                    ...
                                                                    <p className="text-xs mb-1"
                                                                       style={{color: '#6B7280'}}>Total Outlets</p>
                                                                    <p className="text-xl font-bold"
                                                                       style={{color: '#252525'}}>{summary.totalOutlets}</p>
                                                                </div>
                                                                <div
                                                                    className="rounded-md p-4 flex flex-col justify-center"
                                                                    style={{
                                                                    ...
                                                                    <p className="text-xs mb-1"
                                                                       style={{color: '#6B7280'}}>Total Cubes</p>
                                                                    <p className="text-xl font-bold"
                                                                       style={{color: '#252525'}}>{summary.totalCubes}</p>
                                                                </div>
                                                                <div
                                                                    className="rounded-md p-4 flex flex-col justify-center"
                                                                    style={{
                                                                    ...
                                                                    <p className="text-xs mb-1"
                                                                       style={{color: '#6B7280'}}>Total Cases</p>
                                                                    <p className="text-xl font-bold"
                                                                       style={{color: '#252525'}}>{summary.totalCases}</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="flex flex-col items-center justify-center py-16 text-center">
                                            ...
                                            <h3 className="text-lg font-semibold text-gray-900 mb-2">No delivery types
                                                found</h3>
                                ...
                                {activeTab === 'orders' ? (
                                    <table className="w-full">
                                        <thead className="sticky top-0 z-10"
                                               style={{backgroundColor: '#EFEFEF', borderBottom: '1px solid #E3E3E3'}}>
                                        <tr>
                                            <th className="w-12 px-3 py-2.5"
                                                style={{height: '52px', backgroundColor: '#EFEFEF'}}>
                                            ...
                                            <th className="px-3 py-2.5 text-left text-sm font-semibold" style={{
                                                height: '52px',
                                                color: '#252525',
                                        ...
                                        {!orders_loading ? paginatedOrders.map((order) => (
                                            <tr key={order.string} className="hover:bg-gray-50"
                                                style={{borderBottom: '1px solid #E3E3E3'}}>
                                                <td className="px-3 py-2.5" style={{height: '52px'}}>
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedOrders.has(order.string)}
                                                        onChange={() => toggleSelectOrder(order.string)}
                                                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                                    />
                                                </td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{order.string}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{order.dispatcher}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{order.order_count}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{order.capacity}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{
                                                        height: '52px',
                                                        color: '#252525'
                                            ...
                                            <tr>
                                                <td colSpan={9} className="px-3 py-2.5" style={{height: '52px'}}>
                                                    <div className="flex justify-center items-center">
                                                        <div
                                                            className="inline-block animate-spin rounded-full h-6 w-6 border-b-2 border-gray-900">
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                    ...
                                    <table className="w-full">
                                        ...
                                        <tbody className="bg-white">
                                        {!trips_loading ? paginatedTrips.map((trip) => (
                                            <tr key={trip.id} className="hover:bg-gray-50"
                                                ...
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.tripId}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.driver}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.orders}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.outlets}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.volume}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.volumeCrates}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.capacityUsage}</td>
                                                <td className="px-3 py-2.5 text-sm" style={{
                                                    height: '52px',
                                                    color: '#252525'
                                                }}>{trip.plannedDistance}</td>
                                                <td className="px-3 py-2.5 text-sm"
                                                    style={{height: '52px', color: '#252525'}}>{trip.deliveryType}</td>
                                                <td className="px-3 py-2.5" style={{height: '52px'}}>
                          <span
                    ...
                    {activeTab !== 'summary' && (
                        <div className="flex items-center justify-end px-4 py-2 bg-white"
                             style={{borderTop: '1px solid #E3E3E3', gap: '16px'}}>
